...
static bool insideTriangle(int x, int y, const Vector4f* _v){   //判断像素是否在三角形内，高级写法
    Vector3f v[3];
    for(int i=0;i<3;i++)
...
void rst::rasterizer::draw(std::vector<Triangle *> &TriangleList) {
    ...
    Eigen::Matrix4f mvp = projection * view * model;  //mvp坐标转换
    ...
    for (const auto& t:TriangleList)
    {
        ...
        //把点显示在屏幕上，涉及到深度Z-buffer的计算来判断点是否被遮挡，
        //还涉及到顶点的shading计算，并把结果存入framebuffer中等待被调用，
        //在rasterize_triangle函数中完成了这三部分的计算
        ...
        for (float j = bmin; j <= tmax; j++) {
            ...
            if (insideTriangle(i+0.5, j+0.5, t.v)) {   //如果在三角形内
                ...
                if (-z_interpolated < depth_buf[id]) { //如果该像素的深度更小，更新像素深度，颜色表
                    ...
                    depth_buf[id] = -z_interpolated;   //更新该像素点的z-buffer深度z
                    frame_buf[id] = fragment_shader(payload);//使用shader计算颜色
                    // TODO : set the current pixel (use the set_pixel function) to the color of the triangle (use getColor function) if it should be painted.
                    set_pixel({ i,j }, frame_buf[id]);  //设置该像素点
                }
            }
        }
    }
